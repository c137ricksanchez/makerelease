...
class MyApp(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkLabel(self, text="Select the type of release", fg_color="transparent")\
            .grid(row=1, column=0, sticky="w", padx=10, pady=0)

        self.var_type = StringVar(value="Movie (File)")
        options = ["Movie (File)", "Movie (Folder)", "TV Series (Single Season)", "TV Series (Multiple Seasons)"]
        self.option_menu = ctk.CTkOptionMenu(self, variable=self.var_type, values=options)
        self.option_menu.grid(row=2, column=0, sticky="nsew", padx=10, pady=2)

        self.select_button = ctk.CTkButton(self, text="Select Path", command=self.select)
        self.select_button.grid(row=3, column=0, sticky="nsew", padx=10, pady=2)

        self.make_release_button = ctk.CTkButton(self, text="Make Release!", state='disabled', command=self.make_release)
        ...
        self.rename_option_menu = ctk.CTkSwitch(self, text='Rename the file', variable=self.var_rename)
        self.rename_option_menu.grid(row=5, column=0, sticky="nsew", padx=10, pady=2)

        self.author_label = ctk.CTkLabel(self, justify='left',text="Authors: RickSanchez & Norman", fg_color="transparent")
        self.author_label.grid(row=7, column=0, sticky="w", padx=10, pady=0)

        self.check_updates = ctk.CTkLabel(self,justify='left',text="Check for updates", fg_color="transparent", text_color="green")
        self.check_updates.bind("<Button-1>", lambda e: callback("http://www.github.com"))
